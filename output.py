...
def display_results_by_trading_style(topic_companies, topic_model, trading_style='swing'):
    """Display companies ranked by specific trading style score"""
    ...
    print(f"\n{'='*120}")
    print(f"=== COMPANIES RANKED BY {style_name.upper()} ===")
    print(f"{'='*120}")
    ...
    for topic_id in sorted(topic_companies.keys()):
        ...
        # Sort by the selected trading style score
        ...
        companies_with_scores.sort(key=lambda x: x[score_field], reverse=True)
        
        if companies_with_scores:
            print(f"{'Rank':<5} {'Ticker':<8} {'Score':<8} {'Relevance':<10} {'Name':<30} {'Rating':<25} {'Mentions'}")
            print("-" * 120)
            ...
            for rank, comp in enumerate(companies_with_scores[:20], 1):
                ...
                print(f"{rank:<5} {comp['ticker']:<8} {score_str:<8} {relevance_str:<10} {comp['name'][:30]:<30} {category:<25} {mention_info}")
...
def export_multitimeframe_results(topic_companies, topic_model, filename='topic_companies_multitimeframe.csv'):
    """Export results with all timeframe scores"""
    ...
    with open(filename, 'w', newline='', encoding='utf-8') as f:
        ...
        for topic_id in sorted(topic_companies.keys()):
            ...
            for comp in topic_companies[topic_id]:
...
def run_bertopic_with_multitimeframe_scoring(articles, companies_list):
    ...
    topic_companies = match_companies_with_multitimeframe_scores(
        articles, topic_model, companies_list, top_n=TOP_N_COMPANIES
    )
    ...
    # Show top performers by style across all topics
    print(f"\n{'='*120}")
    print("=== TOP 10 STOCKS BY TRADING STYLE (Across All Topics) ===")
    print(f"{'='*120}")
    ...
    for style in trading_styles:
        ...
        print(f"\n🎯 {style_name.upper()}:")
        print(f"{'Rank':<5} {'Ticker':<8} {'Score':<8} {'Relevance':<10} {'Topic':<8} {'Company':<30} {'Rating'}")
        print("-" * 120)
        for rank, comp in enumerate(all_companies[:10], 1):
            print(f"{rank:<5} {comp['ticker']:<8} {comp['score']:.3f}    {comp['relevance']:.3f}      {comp['topic_id']:<8} {comp['name'][:30]:<30} {comp['category']}")
